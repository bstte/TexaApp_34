...
import { View, Text, ScrollView, ActivityIndicator, Image, TouchableOpacity, TextInput, KeyboardAvoidingView, Platform, TouchableWithoutFeedback, Keyboard } from 'react-native';
...
const ViewQuery = ({ route }) => {
  ...
  const handleSubmitReplyForm = async () => {
    ...
    ReplyData.append('role', role);
    if (role === 1) {
      ReplyData.append('status', selectedStatus);
    } else {
      ReplyData.append('status', ''); 
    }
    if (ReplyImage) {
    ...
    if (token) {
      ...
      try {
        ...
        handleApiError(error, "Query reply error:")
        //  console.log(error,)
      }
    }

...
const AdminRaiseQueryCard: React.FC<RaiseQuerydprops> = ({ props, index, onDelete, navigation, represantative }) => {

    const [ChatConting, SetChatCounting] = useState(0)
    const userData = useSelector((state: any) => state.user.userData)
    const adminId = userData && userData.id ? userData.id : '';
    const [value, setValue] = useState(props.reprsentetive_id);
    ...
    const representativeData = represantative.map((representative) => ({
        label: representative.name + (representative.lname ? representative.lname : ""),
    ...
    const Assignreparesentative = async (value, case_id) => {
        setValue(value)
        try {
            const token = await AsyncStorage.getItem('token');
            const query = {
            ...
            // console.log("send form", query)

            if (token && query) {
                const response = await api.assignquery_to_reprsentative(query, token, value)
              ...
              
                {
                    role === 1 && user_type === 0 ? (
                        <Dropdown
                            ...
                            value={value}
                            onChange={item => {
                                Assignreparesentative(item.value, props.case_id)
...
export default AdminRaiseQueryCard
